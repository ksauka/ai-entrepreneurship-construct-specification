...
from aecsp.corpus.query_provenance import (
    QUERY_BY_ID,
    QUERY_COUNT_COLUMN,
...
# NOTE: we deliberately do NOT add lowercase alias columns (title, source_title,
# ...). They duplicate the Scopus headers (Title, Source title, ...) and, because
# VOSviewer normalizes headers (lowercase + strip punctuation), the pair collapses
# to one name and VOSviewer rejects the file as having duplicate columns. The
# single stable ETV identifier we add is ``paper_id`` (no Scopus collision).
