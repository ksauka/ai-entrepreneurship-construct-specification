...
Usage:
  ...
  bash scripts/backup_review_host.sh INCOMPLETE_BUNDLE --resume-incomplete [--include-project-env]
...
the tunnel token are encrypted into one OpenSSL AES-256 archive unless
--no-secrets is used.

--resume-incomplete reuses an existing runtime.tar after an interrupted final
encryption or checksum step. It refreshes the offline Git bundle and recorded
commit to the current clean checkout before finalizing the bundle.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --resume-incomplete) RESUME_INCOMPLETE=1 ;;
    -h|--help) usage; exit 0 ;;
...
if [[ "$RESUME_INCOMPLETE" -eq 1 ]]; then
  if [[ ! -d "$DESTINATION" ]]; then
    echo "Incomplete bundle does not exist: $DESTINATION" >&2
    exit 1
  fi
  BUNDLE_ROOT="$(cd "$DESTINATION" && pwd)"
else
  mkdir -p "$DESTINATION"
...
if [[ "$RESUME_INCOMPLETE" -eq 0 && -e "$BUNDLE_ROOT" ]]; then
...
if [[ "$RESUME_INCOMPLETE" -eq 1 ]]; then
  if [[ ! -s "$BUNDLE_ROOT/runtime.tar" ]]; then
    echo "Incomplete bundle has no usable runtime.tar: $BUNDLE_ROOT" >&2
    exit 1
  fi
  git -C "$PROJECT_ROOT" bundle create \
    "$BUNDLE_ROOT/repository.bundle.pending" --all
  mv "$BUNDLE_ROOT/repository.bundle.pending" \
...
if [[ "$INCLUDE_SECRETS" -eq 1 ]]; then
  if ! command -v openssl >/dev/null 2>&1; then
    echo "openssl is required to encrypt host credentials." >&2
    exit 1
  fi
  if [[ ! -t 0 ]]; then
    echo "Secret encryption requires an interactive terminal." >&2
    exit 1
  fi
  ...
  openssl enc -aes-256-cbc -salt -pbkdf2 -iter 250000 \
    -in "$SECRET_STAGE/host-secrets.tar.gz" \
    -out "$BUNDLE_ROOT/host-secrets.tar.gz.enc"
  ...
  chmod 600 "$BUNDLE_ROOT/host-secrets.tar.gz.enc" 2>/dev/null || true
...
echo "  $BUNDLE_ROOT"
du -sh "$BUNDLE_ROOT"
echo "Keep the encryption passphrase separately from the bundle."
